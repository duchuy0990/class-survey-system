...
@section('content')
<div class="card-columns tc-list-class">
    @foreach ($classes as $class)
        <div class="card border-dark mb-3">
            @if ($class->total_sv == $class->da_danh_gia)
                <div class="card-header survey-stt">
                    <small class="text-muted survey-done"></small>
                    <a href="{{url('teacher/result/'.$class->ma_mh)}}" target="_blank" class="btn btn-success" style="margin-top:0">Xem kết quả</a>
                </div>
            @else
                <div class="card-header survey-stt">
            ...
            <div class="card-body text-dark">
                ...
                <p class="card-text" id="count-cpl">Đã đánh giá: {{$class->da_danh_gia}}/{{$class->total_sv}}</p>
            </div>
        </div>
    @endforeach
</div>
